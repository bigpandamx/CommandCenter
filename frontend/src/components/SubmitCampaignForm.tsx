  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const aliasList = aliases
        ...
        .map((a) => a.trim())
        .filter((a) => a.length > 0);
      const response = await fetch("/api/threat-intel/campaigns", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        setError(body.message ?? "Couldn't add the campaign.");
        return;
      }
      setName("");
      setDescription("");
    ...
    <form onSubmit={handleSubmit} className="space-y-2 rounded-lg border border-border bg-surface p-3">
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g. an operation observed locally, not yet in MITRE's own catalog"
        />
      </div>
      <div>
