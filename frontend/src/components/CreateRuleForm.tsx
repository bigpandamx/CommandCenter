  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(body.message ?? body.error ?? "Couldn't create the rule.");
        return;
      }
      router.push(`/compliance/rules/${key}`);
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <form onSubmit={handleSubmit} className="mt-6 max-w-xl space-y-4">
      ...
      <div>
        ...
        <input
          ...
          placeholder="AI Transparency Rule"
        />
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="mt-1 w-full rounded border border-border bg-surface px-3 py-1.5 text-sm text-text-primary"
          placeholder="What this regulatory topic covers."
        />
      </div>
