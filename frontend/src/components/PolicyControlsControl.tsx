...
export function PolicyControlsControl({
  policyKey,
  implemented,
  allControls,
}: {
  policyKey: string;
  implemented: ControlOption[];
  ...
  const implementedKeys = new Set(implemented.map((c) => c.key));
  const candidates = allControls.filter((c) => !implementedKeys.has(c.key));
  ...
  async function handleAdd() {
    ...
    setPending(true);
    try {
      const response = await fetch(`/api/governance/policies/${policyKey}/controls`, {
        method: "POST",
  ...
  async function handleRemove(controlKey: string) {
    ...
    setPending(true);
    try {
      await fetch(`/api/governance/policies/${policyKey}/controls/${controlKey}/remove`, { method: "POST" });
      router.refresh();
    } finally {
      setPending(false);
    }
  ...
  return (
    <div>
      ...
      {implemented.length === 0 ? (
        <p className="text-sm text-text-muted">No controls implemented yet.</p>
      ) : (
      ...
      {candidates.length > 0 && (
        <div className="mt-2 flex gap-2">
          ...
          >
            <option value="">Implement a control…</option>
            {candidates.map((c) => (
