...
export function PackControlsControl({
  packKey,
  bundled,
  allControls,
}: {
  packKey: string;
  bundled: ControlOption[];
  ...
  const bundledKeys = new Set(bundled.map((c) => c.key));
  const candidates = allControls.filter((c) => !bundledKeys.has(c.key));
  ...
  async function handleAdd() {
    ...
    setPending(true);
    try {
      const response = await fetch(`/api/compliance/packs/${packKey}/controls`, {
        method: "POST",
  ...
  async function handleRemove(controlKey: string) {
    ...
    setPending(true);
    try {
      await fetch(`/api/compliance/packs/${packKey}/controls/${controlKey}/remove`, { method: "POST" });
      router.refresh();
    } finally {
      setPending(false);
    }
  ...
  return (
    <div>
      ...
      {bundled.length === 0 ? (
        <p className="text-sm text-text-muted">No controls bundled yet.</p>
      ) : (
      ...
      {candidates.length > 0 && (
        <div className="mt-2 flex gap-2">
          ...
          >
            <option value="">Bundle a control…</option>
            {candidates.map((c) => (
